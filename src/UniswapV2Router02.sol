...
contract UniswapV2Router02 {
    ...
    function _addLiquidity(
        ...
        uint256 amountADesired, // 用户希望投入的 A 数量
        uint256 amountBDesired, // 用户希望投入的 B 数量
        uint256 amountAMin, // 最小接受的 A 数量（滑点保护）
        uint256 amountBMin // 最小接受的 B 数量（滑点保护）
    ...
    function addLiquidity(
        ...
        uint256 amountADesired, // 希望投入的 A 数量
        uint256 amountBDesired, // 希望投入的 B 数量
        uint256 amountAMin, // 最小投入 A（滑点保护）
        uint256 amountBMin, // 最小投入 B（滑点保护）
        ...
        returns (
            uint256 amountA, // 实际投入的 A 数量
            uint256 amountB, // 实际投入的 B 数量
            uint256 liquidity // 铸造的 LP 代币数量
        )
    {
        // 1. 计算实际要投入的 A 和 B
    ...
    function addLiquidityETH(
        ...
        uint256 amountTokenDesired, // 希望投入的 token 数量
        uint256 amountTokenMin, // 最小投入 token（滑点保护）
        uint256 amountETHMin, // 最小投入 ETH（滑点保护）
        address to, // LP 代币接收地址
        uint256 deadline // 截止时间
    )
        external
        ...
        returns (
            uint256 amountToken, // 实际投入的 token 数量
            uint256 amountETH, // 实际包裹并投入的 ETH 数量
            uint256 liquidity // 铸造的 LP 代币数量
        )
    {
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        uint256 liquidity, // 用户希望销毁的 LP 代币数量
        uint256 amountTokenMin, // 最小可接受的 ERC20 代币数（滑点保护）
        uint256 amountETHMin, // 最小可接受的 ETH 数量（滑点保护）
        address to, // 最终接收取回资产的地址
        uint256 deadline // 截止时间戳，过期则 revert
    ...
    function removeLiquidityETHWithPermitSupportingFeeOnTransferTokens(
        ...
        uint256 liquidity, // 要销毁的 LP 代币数量
        uint256 amountTokenMin, // 最小接受的 ERC20 数量
        uint256 amountETHMin, // 最小接受的 ETH 数量
    ...
    function _swap(
        uint256[] memory amounts, // 按路径计算出的每一步的输入/输出数量
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut) // input 为 token0，则输出到 token1
                : (amountOut, uint256(0)); // input 为 token1，则输出到 token0
            // 确定本跳 swap 的接收地址：如果不是最后一跳，接收方是下一个 Pair
            address to = i < path.length - 2 ? UniswapV2Library.pairFor(factory, output, path[i + 2]) : _to; // 最后一跳则直接发给 _to
    ...
    function swapExactTokensForTokens(
        uint256 amountIn, // 精确的输入数量
        uint256 amountOutMin, // 最小可接受的输出数量（滑点保护）
    ...
    function swapTokensForExactTokens(
        uint256 amountOut, // 精确的目标输出数量
        uint256 amountInMax, // 最大可消耗输入数量
        address[] calldata path, // 兑换路径
        address to,
        uint256 deadline
    ) external virtual ensure(deadline) returns (uint256[] memory amounts) {
    ...
    function swapExactETHForTokens(
        uint256 amountOutMin, // 最小可接受的输出数量
    ...
    function swapTokensForExactETH(
        uint256 amountOut, // 精确的目标 ETH 输出
        uint256 amountInMax, // 最大可消耗的输入代币数量
    ...
    function swapExactTokensForETH(
        ...
        uint256 amountOutMin, // 最小可接受的 ETH 输出
        address[] calldata path, // 路径，末位必须为 WETH
        address to,
        uint256 deadline
    ) external virtual ensure(deadline) returns (uint256[] memory amounts) {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        uint256 amountIn, // 精确的输入数量
        uint256 amountOutMin, // 最小可接受的最终输出（滑点保护）
        address[] calldata path, // 兑换路径
        address to, // 最终接收地址
        uint256 deadline // 截止时间
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint256 amountOutMin, // 最小可接受输出数量
